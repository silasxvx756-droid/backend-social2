...
const fetchClerkUsers = async (query) => {
  try {
    const response = await fetch(`https://api.clerk.dev/v1/users?limit=50&query=${encodeURIComponent(query)}`, {
...
app.get("/users/:username", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      user = await User.create({ id: u.clerkId, username: u.username, displayName: u.displayName, avatar: u.avatar });
    }
...
app.get("/followers/:clerkId", async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: "Erro ao buscar seguidores" }); }
});
...
app.get("/posts", async (req, res) => {
  try {
    ...
    const updatedPosts = await Promise.all(posts.map(async (post) => {
      ...
      return { ...post.toObject(), actor: freshUser || post.actor };
    }));
...
app.post("/posts/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    const finalActor = freshUser || actor;
    let imageUrl = null;
    if (req.file) {
      const result = await new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ folder: "posts" }, (err, result) => (result ? resolve(result) : reject(err)));
...
app.get("/posts/:id", async (req, res) => {
  try { const post = await Post.findById(req.params.id); if (!post) return res.status(404).json({ error: "Post não encontrado" }); res.json(post); } catch (err) { res.status(500).json({ error: "ID inválido" }); }
});
...
app.post("/posts/:id/like", async (req, res) => {
  try {
    ...
    else { post.likes.push({ id: user.id, username: user.username }); await Notification.create({ userId: post.actor.id, type: "like", postId: String(post._id), actor: user }); }
    await post.save();
...
app.post("/posts/:id/comments", async (req, res) => {
  try {
    ...
    await post.save();
    await Notification.create({ userId: post.actor.id, type: "comment", postId: String(post._id), actor: user });
    res.json(comment);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/notifications/:userId", async (req, res) => {
  ...
  const updatedNotifications = await Promise.all(notifications.map(async (n) => {
    ...
    return { ...n.toObject(), actor: freshActor || n.actor };
  }));
